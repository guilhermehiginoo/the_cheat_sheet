...
parked directory is served automatically at `<folder>.test` — no `serve`
command, no port numbers.
...
- Running a bare `php artisan migrate` on the host while using Sail is a common
  mistake — it targets the host's PHP and can't reach the container's database.
...
- In development, keep them cleared — cached config ignores `.env` changes.
